...
const mediaConnections: Map<string, MediaConnection> = new Map();
...
if (call) {
mediaConnections.set(peerId, markRaw(call));
...
function handleCallRequest(_peerId: string, _data: any) {
...
localStream.value = stream;
const call = mediaConnections.get(appStore.callState.peerId);
if (call) {
call.answer(stream);
setupCallListeners(call);
appStore.updateCallState({

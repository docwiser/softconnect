  ...
  <div class="meeting-room" id="main-content" tabindex="-1" @keydown.escape="handleEscape">
    ...
    <main class="room-main" :class="`layout-${meetingStore.layout}`" aria-label="Meeting video area">
      ...
      <div v-if="meetingStore.layout === 'grid'" class="video-grid" :class="`grid-${gridClass}`" role="list" aria-label="Participants">
        <!-- Local tile -->
        <div class="video-tile local-tile" role="listitem"
          :aria-label="`You${meetingStore.isAudioMuted ? ', muted' : ''}${meetingStore.isVideoOff ? ', camera off' : ''}`">
          <div class="tile-video-wrapper">
            <video ref="localVideoRef" class="tile-video" autoplay muted playsinline
              :class="{ hidden: meetingStore.isVideoOff }" aria-label="Your video"></video>
        ...
        >
          ...
          >
            <button class="host-overlay-btn" @click.stop="meetingStore.hostMute(rs.uid)" :aria-label="`Mute ${rs.displayName}`" title="Mute">
            ...
            <button class="host-overlay-btn" @click.stop="meetingStore.setSpotlight(rs.uid)" :aria-label="`Spotlight ${rs.displayName}`" title="Spotlight">
            ...
            <button class="host-overlay-btn danger" @click.stop="meetingStore.hostRemoveParticipant(rs.uid)" :aria-label="`Remove ${rs.displayName}`" title="Remove">
      ...
      <div v-else-if="meetingStore.layout === 'spotlight'" class="spotlight-layout" aria-label="Spotlight view">
        <div class="spotlight-main">
          <div v-if="spotlightStream" class="spotlight-video-wrap">
            <RemoteVideoTile
              v-if="spotlightStream.uid !== 'local'"
              :stream="spotlightStream.stream"
              :is-video-off="spotlightStream.isVideoOff"
            />
      ...
      >
        ...
        <button @click="meetingStore.hostLowerHand(meetingStore.handRaisedQueue[0])" :aria-label="`Lower hand for ${getParticipantName(meetingStore.handRaisedQueue[0])}`">Lower</button>
      </div>
      ...
      <!-- Participants panel -->
      <Transition name="panel-slide">
        <aside v-if="meetingStore.isParticipantsPanelOpen" class="participants-panel" aria-label="Participants list" role="complementary">
          <div class="panel-header">
          ...
          <div v-if="meetingStore.canManageParticipants" class="host-controls-bar" role="group" aria-label="Host controls">
            <button class="host-bar-btn" @click="meetingStore.hostMuteAll()" aria-label="Mute all participants">Mute All</button>
            <button class="host-bar-btn" @click="meetingStore.hostUpdateSettings({ allowParticipantAudio: !meetingStore.meeting?.settings.allowParticipantAudio })"
          ...
          <ul class="participants-list" role="list">
            ...
            >
              <div class="part-avatar" aria-hidden="true">
                ...
                <span v-if="remoteStreams.get(p.uid)?.isSpeaking" class="speaking-ring" aria-hidden="true"></span>
              </div>
              <div class="part-info">
    ...
    <div class="meeting-notifications" aria-live="polite" aria-atomic="false">
      <TransitionGroup name="notif">
        <div v-for="notif in meetingStore.notifications" :key="notif.id" :class="['meeting-notif', `notif-${notif.type}`]" role="status">
          {{ notif.message }}
        </div>
      </TransitionGroup>
    </div>
    ...
    <TransitionGroup name="reaction" class="reaction-overlay" tag="div" aria-hidden="true">
      <div v-for="r in reactions" :key="r.id" class="reaction-bubble" :style="{ left: r.x + '%' }">{{ r.emoji }}</div>
    </TransitionGroup>
    ...
    <nav class="controls-bar" role="toolbar" aria-label="Meeting controls">
      ...
      <div class="ctrl-group ctrl-center" role="group" aria-label="Meeting actions">
        ...
        <div class="ctrl-reaction-wrap">
          <button class="ctrl-btn" @click="showReactionPicker = !showReactionPicker"
            :aria-label="showReactionPicker ? 'Close reactions' : 'Send a reaction'" :aria-expanded="showReactionPicker">
            <span aria-hidden="true">😊</span>
          ...
          <div v-if="showReactionPicker" class="reaction-picker" role="listbox" aria-label="Choose a reaction">
            <button v-for="e in reactionEmojis" :key="e" class="reaction-btn" @click="sendReaction(e)" :aria-label="e" role="option">{{ e }}</button>
          </div>
        </div>
...
// Inline RemoteVideoTile — attaches MediaStream to a <video> element
...
<!-- Styles unchanged from original MeetingRoomScreen.vue — copied verbatim -->
<style scoped>
...
.video-tile { background:#0f1126;border-radius:10px;overflow:hidden;position:relative;aspect-ratio:16/9;border:2px solid transparent;transition:border-color .2s;cursor:default; }
...
.chat-input-area textarea::placeholder { color:rgba(255,255,255,.25); }
...
.host-bar-btn { background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:.375rem .75rem;color:rgba(255,255,255,.7);font-family:inherit;font-size:.78rem;font-weight:500;cursor:pointer;transition:all .15s;min-height:32px; }
.host-bar-btn:hover { background:rgba(255,255,255,.12);color:#fff; }
...
.part-avatar { width:36px;height:36px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#5c3bff40,#ff3b8c40);display:flex;align-items:center;justify-content:center;font-size:1rem;overflow:hidden;position:relative; }
.part-avatar img { width:100%;height:100%;object-fit:cover; }
.speaking-ring { position:absolute;inset:-3px;border-radius:50%;border:2px solid #34d399;animation:speakRing .8s ease-in-out infinite alternate; }
@keyframes speakRing { 0%{opacity:.4}100%{opacity:1} }
...
.part-action-btn:hover { background:rgba(255,255,255,.12);color:#fff; }
...
.participant-menu button { display:block;width:100%;padding:.625rem 1rem;background:none;border:none;color:rgba(255,255,255,.72);cursor:pointer;font-family:inherit;font-size:.82rem;text-align:left;transition:background .15s;min-height:36px; }
...
.remove-item:hover { background:rgba(255,59,92,.15)!important; }
...
.device-field label { font-size:.8rem;font-weight:600;color:rgba(255,255,255,.55);letter-spacing:.04em; }
...
.device-field select:focus { outline:none;border-color:rgba(92,59,255,.6); }
.setting-toggle-row { display:flex;align-items:center;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid rgba(255,255,255,.05); }
.setting-toggle-row label { font-size:.85rem;color:rgba(255,255,255,.65);cursor:pointer; }
.setting-toggle-row input[type=checkbox] { width:36px;height:20px;border-radius:999px;appearance:none;-webkit-appearance:none;background:rgba(255,255,255,.15);transition:background .2s;cursor:pointer;position:relative;flex-shrink:0; }
...
.reaction-bubble { position:absolute;bottom:80px;font-size:2.5rem;animation:floatUp 3s ease-out forwards; }
@keyframes floatUp { 0%{transform:translateY(0) scale(1);opacity:1}100%{transform:translateY(-300px) scale(1.5);opacity:0} }
.reaction-enter-active,.reaction-leave-active { transition:all .3s; }
...
.ctrl-btn.off:hover { background:rgba(255,59,92,.38); }
.ctrl-btn.active { background:rgba(92,59,255,.3);color:#a78bfa; }
.ctrl-btn.active:hover { background:rgba(92,59,255,.45); }
...
.leave-btn { background:rgba(255,59,92,.2);color:#ff6b8a; }
.leave-btn:hover { background:rgba(255,59,92,.35);color:#fff; }
...
@media(max-width:640px){
  ...
  .ctrl-btn{min-width:48px;padding:.375rem .5rem}
}
</style>
